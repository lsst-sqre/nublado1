...
def load_jupyter_server_extension(nbapp):
    ...
    page_config['exposeAppInBrowser'] = getattr(
        nbapp, 'expose_app_in_browser', False)
    ...
    # Make sure the app dir exists.
        ...
        if msgs:
            ...
            handler = (ujoin(base_url, '/lab'),
                       ErrorHandler, {'messages': msgs})
    ...
    # FIXME
    import pprint
    logger.debug("nbapp attrs: {}".format(pprint.pformat(vars(nbapp))))
    if hasattr(nbapp, 'hub_prefix'):
        logger.debug("Setting builtin Hub page_config values.")
    ...
    # I don't understand what promoted the hub settings to top level but
    #  not the Rubin settings, but w/e
    if (hasattr(nbapp, '_trait_values')):
        import pprint
        ntv = nbapp._trait_values
        logger.debug("NTV: {}".format(pprint.pformat(ntv)))
        nts = ntv.get('tornado_settings')
        if nts:
            logger.debug("NTS: {}".format(pprint.pformat(nts)))
            hapi = nts.get('rubin_hub_api_url')
            if hapi:
                logger.debug("Setting Rubin page_config values.")
                page_config['rubinHubApiUrl'] = hapi
                page_config['rubinHubUser'] = nts['rubin_hub_user']
                page_config['rubinHubApiToken'] = nts['rubin_hub_api_token']
                # These next are kind of redundant
                page_config['rubinHubApiHost'] = nts['rubin_hub_api_host']
                page_config['rubinHubApiPath'] = nts['rubin_hub_api_path']
                page_config['rubinHubApiScheme'] = nts['rubin_hub_api_scheme']
                page_config['rubinHubApiPort'] = nts['rubin_hub_api_port']
                api_token = os.getenv('JUPYTERHUB_API_TOKEN', '')
                page_config['rubinHubApiToken'] = api_token
    # FIXME want to display it
    logger.debug("Page Config: {}".format(page_config))
